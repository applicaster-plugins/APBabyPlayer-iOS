        ...
        if let configurationJson = configurationJSON {
            ...
            if let playerType = configurationJson.object(forKey: "player_controller") as? String, playerType.isNotEmptyOrWhiteSpaces() {
                if playerType == "advanced_player" {
                    if let playerControlsView = (Bundle(for: self).loadNibNamed("GAPlayerControlsViewType1", owner: self, options: nil)?.first) as? UIView & APPlayerControls {
                        
                        if let shouldShowInstructionMessage = configurationJson.object(forKey: "should_show_instruction_message") as? String,
                            let playerControlsViewType1 = playerControlsView as? GAPlayerControlsViewType1 {
                            playerControlsViewType1.shouldShowInstructionMessage = shouldShowInstructionMessage.boolValue()
                        }
                    ...
                    if let playerControlsView = (Bundle(for: self).loadNibNamed("GAParentLockPlayerControlsType2", owner: self, options: nil)?.first) as? GAParentLockPlayerControlsType2 {
